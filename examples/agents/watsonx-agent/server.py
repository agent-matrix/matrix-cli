...
PORT       = int(os.getenv("WATSONX_AGENT_PORT", 6288))
...
# Optional /health route (non-invasive). Works if FastMCP exposes an ASGI app.
try:
    from starlette.responses import PlainTextResponse  # FastMCP uses FastAPI/Starlette under the hood
except Exception:
    PlainTextResponse = None

_app = getattr(mcp, "app", None)
if _app is not None and hasattr(_app, "get"):
    @_app.get("/health")
    async def _health():
        # Plain text is convenient for curl and simple probes; JSON also fine if PlainTextResponse missing
...
if __name__ == "__main__":
    logging.info(f"Starting Watsonx MCP server at http://127.0.0.1:{PORT}/sse (health: /health)")

...
# Print a command, capture its output & status, and show the output block.
cmd_show() {
  printf "${C_CMD}$ %s${C0}\n" "$*" >&2
...
if [[ ${#ENV_PREFIX[@]} -gt 0 ]]; then
  cmd_show env "${ENV_PREFIX[@]}" matrix run "$ALIAS" --port "$PORT" || true
else
...
printf "${C_CMD}$ matrix mcp call chat --url %s --args '%s'${C0}\n" "$SSE_URL" "$PAYLOAD"
CALL_OUT="$(matrix mcp call chat --url "$SSE_URL" --args "$PAYLOAD" 2>&1)"
printf "${C_DIM}%s${C0}\n" "$CALL_OUT"

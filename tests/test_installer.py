...
def _patch_common_cli_bits(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cli_mod, "load_config", lambda: _DummyCfg("http://hub", "tok", str(tmp_path)))

    # client_from_config → return our dummy client
    monkeypatch.setattr(cli_mod, "client_from_config", lambda cfg: _DummyClient(cfg.hub_base, cfg.token))
    ...
    class _Resolved:
        def __init__(self, fqid, note=None): self.fqid, self.note = fqid, note
    monkeypatch.setattr(cli_mod, "resolve_fqid", lambda *a, **k: _Resolved("mcp_server:watsonx-agent@0.1.0"))

    # AliasStore → in-memory stub to avoid touching user files
    alias_mod = types.SimpleNamespace()
    class _AliasStore:
        _db = {}
        def get(self, alias): return self._db.get(alias)
        def set(self, alias, *, id, target): self._db[alias] = {"id": id, "target": target}
    ...
    monkeypatch.setattr(sdk_inst.LocalInstaller, "prepare_env", lambda *a, **k: None, raising=False)
    ...
    monkeypatch.setattr(sdk_inst.LocalInstaller, "materialize", _fake_materialize, raising=False)
    ...
    monkeypatch.setattr(sdk_inst.LocalInstaller, "_load_runner_from_report", _fake_load, raising=False)
...
def test_cli_install_writes_runner_json_from_flag(tmp_path, monkeypatch):
    ...
    args = [
        ...
        "--runner-url", "https://example.test/runner.json",
        "--force", "--no-prompt",
    ]
...
def test_cli_install_repo_url_triggers_fake_clone(tmp_path, monkeypatch):
    ...
    def _fake_tempdir():
        class _Ctx:
            def __init__(self, base): self.name = str(base)
            def __enter__(self):
    ...
    def _fake_run(cmd, *args, **kwargs):
        ...
        (tmpd / "bin" / "run.py").write_text("#!/usr/bin/env python3\nprint('ok')\n", encoding="utf-8")
    ...
    args = [
        ...
        "--target", str(target_dir),
        "--runner-url", "https://example.test/runner.json",
        "--repo-url", "https://github.com/ruslanmv/watsonx-mcp.git",
        "--force", "--no-prompt",
    ]
    result = runner.invoke(install_app, args)
    ...
    assert entry.is_file(), "--repo-url should have cloned (simulated) files into target"

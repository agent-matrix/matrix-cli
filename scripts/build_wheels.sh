#!/usr/bin/env bash
#
# build_wheels.sh — build local wheel(s) for matrix-cli for testing (no PyPI publish)
#
# Usage:
#   ./build_wheels.sh              # builds in current project dir
#   PACKAGE_DIR=./packages/cli ./build_wheels.sh
#
# Outputs wheels and sdist under ./dist and copies them to ./wheelhouse
# You can then install on another machine via:
#   pip install --no-index --find-links=wheelhouse matrix-cli==<version>
# or directly:
#   pip install wheelhouse/matrix_cli-<ver>-py3-none-any.whl
...
# Ensure build tooling is present (user env)
...
Next steps (install from wheelhouse):
  ...
  # Install without touching PyPI (SDK wheel must also be present if required)
  python -m pip install --no-index --find-links=wheelhouse matrix-cli

  # Or install a specific built file
  python -m pip install wheelhouse/matrix_cli-*-py3-none-any.whl
